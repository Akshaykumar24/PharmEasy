...
//click right btn and slide should move left 1 unit
/*
...
*/
//better way to do the same and much efficient
...
// if i had navigation dots also uncomment the following
/*
dots.addEventListener("click", (e) => {
  const targetdot = e.target.closest("button");
  if (!targetdot) return;

  //dots are inside dotnav
  // current dot will have the class of current same as li of carousel

  const current = track.querySelector(".current");
  const currentdot = dotnav.querySelector(".current");
  const targetindex = dots.findIndex((dot) => dot === targetdot);
  const targetslide = slide[targetindex];
  moveit(track, current, targetslide);
});
*/
